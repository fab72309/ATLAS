    ...
    <div className="w-full min-w-0 overflow-y-auto rounded-3xl border border-slate-200/80 bg-white/88 p-4 shadow-[0_16px_40px_-28px_rgba(15,23,42,0.28)] sm:p-5 dark:border-white/10 dark:bg-white/5 dark:shadow-none">
            ...
            <div className="space-y-3 pt-3">
              ...
              {filteredSelected.map((s) => {
                ...
                const statusBorderClass = isRequested
                  ? `border-dashed ${colorMeta.dashedClass || 'border-slate-300/70 dark:border-slate-400/80'}`
                  : 'border-solid';
                return (
                  <div
                    key={s.id}
                    className={`space-y-2 rounded-2xl border border-solid p-3 ${colorMeta.color || 'border-slate-200'} ${colorMeta.fill || 'bg-slate-50/80'} shadow-sm`}
                  >
                    <div className="space-y-2">
                      <div className={`inline-flex w-fit max-w-full items-center rounded-lg border px-3 py-2 text-sm font-semibold leading-tight ${statusBorderClass} ${colorMeta.color || 'border-slate-200 bg-white text-slate-700 dark:border-white/20 dark:bg-white/5 dark:text-white'}`}>
                        {s.name}
                      </div>
                      <div className="grid w-full grid-cols-1 items-center gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto]">
                        <div className="relative min-w-0 w-full">
                          <select
                            value={assignSelection[s.id] || assignableNodes[0]?.id || ''}
                            onChange={(e) => setAssignSelection((prev) => ({ ...prev, [s.id]: e.target.value }))}
                            className="h-9 w-full rounded-lg border border-solid border-slate-200 bg-white px-3 text-sm text-slate-700 transition focus:border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-200/80 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:focus:border-white/20 dark:focus:ring-white/20 dark:disabled:bg-white/[0.03] dark:disabled:text-gray-500"
                        ...
                        <button
                          ...
                          className={`inline-flex h-9 min-w-20 items-center justify-center whitespace-nowrap rounded-lg border px-2.5 text-xs font-semibold text-white transition ${
                            isAssigned
                              ? 'bg-blue-600 border-blue-300/70 text-blue-50'
                        ...
                        <div
                          className="inline-flex w-fit items-center gap-2 rounded-md border border-slate-200 bg-white px-2 py-1 dark:border-white/10 dark:bg-white/5"
                          role="radiogroup"
    ...
    <div className="w-full min-w-0 overflow-y-auto rounded-3xl border border-slate-200/80 bg-white/88 p-4 shadow-[0_16px_40px_-28px_rgba(15,23,42,0.28)] sm:p-5 dark:border-white/10 dark:bg-white/5 dark:shadow-none">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-md border border-blue-400/40 bg-blue-600 px-2 py-1 text-[11px] text-white transition hover:bg-blue-500"
              >
            ...
            {!sectors.length && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-400">
                  ...
                  <div key={sector.id} className="min-w-0 w-full max-w-full space-y-2 rounded-2xl border border-slate-200/80 bg-white/72 p-3 dark:border-white/10 dark:bg-black/20">
                    <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-1.5">
                      <div className="relative min-w-0 w-full">
                        <input
                          ...
                          className={`w-full rounded-md border border-slate-200 bg-white px-2 py-1.5 pr-8 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200/80 dark:border-white/10 dark:bg-white/5 dark:focus:ring-white/20 ${
                        ...
                        {sectorValidated[sector.id] && (
                          <Check className="absolute right-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-blue-400" />
                        )}
                      </div>
                      <button
                        onClick={() => {
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`h-3 w-3 text-slate-500 transition-transform dark:text-gray-300 ${subsectorOpen[sector.id] ? 'rotate-180' : 'rotate-0'}`}
                        />
                      </button>
                    </div>
                    <div className="space-y-1.5">
                      {subsectorOpen[sector.id] && (
                        <>
                          {!subsectors.length && (
                            <div className="rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5 text-[11px] text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-500">
                              Aucun sous-secteur.
                            </div>
                          )}
                          {subsectors.map((sub) => (
                            <div key={sub.id} className="flex flex-wrap items-center gap-1.5">
                              <input
                                ...
                                className="w-full rounded-md border border-slate-200 bg-white px-2 py-1.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200/80 dark:border-white/10 dark:bg-white/5 dark:text-white dark:focus:ring-white/20 sm:w-56"
                          ...
                          <button
                            ...
                            className="flex w-full items-center justify-center gap-2 rounded-md border border-dashed border-slate-200 bg-slate-50 px-2.5 py-1.5 text-[11px] text-slate-600 transition hover:bg-slate-100 dark:border-white/20 dark:bg-white/5 dark:text-gray-200 dark:hover:bg-white/10"
                          >
                            <Plus className="h-3.5 w-3.5" />
                    ...
                    <div key={`${catKey}-${section.id}`} className="space-y-2">
                      ...
                      <div className="flex flex-wrap items-start gap-2">
                        {section.items.map((m) => {
                          ...
                          const statusBorderClass = isRequested
                            ? `border-dashed ${meta.dashedClass}`
                            : 'border-solid';
                          ...
                          return (
                            <div
                              ...
                              className={`relative w-fit max-w-full sm:max-w-[18rem] flex min-h-[44px] flex-col items-center justify-center rounded-lg border px-4 pr-10 py-2 text-center leading-tight cursor-pointer ${statusBorderClass} ${already ? meta.dashedClass : 'border-slate-200 dark:border-white/15'} ${meta.color} hover:bg-slate-100 dark:hover:bg-white/10 transition`}
                            >
                              <button
                                type="button"
                                onClick={(event) => {
